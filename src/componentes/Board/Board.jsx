                    ...
                    {gridItems.map((item) => {
                        if (item.content == null) {
                            ...
                            return (
                                <div
                                    key={item.id}
                                    className={`${styles.grid_item} ${setClass(item.sizeShip)}`}
                                    id={`item-${item.id}`}>
                                    {item.content}
                                </div>
                            );
            ...
            <div>
                <div className={styles.container}>
                    ...
                    {remainingShips.map((quantity, index) => (
                        <label
                            key={index}
                            style={{ marginBottom: "5px" }}>{`Quantidade do navio ${index}: ${quantity}`}</label>
                    ))}

                    <button className={styles.btn} onClick={() => shipsPosition(ship, Number(position))}>
                        Adicionar navio
                    </button>
                    {/* Concluído button */}

                    <button type="submit" className={styles.con} onClick={() => submitShips()}>
                        Concluído
                    </button>
                </div>
            </div>
